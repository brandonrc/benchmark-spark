...
# Tracks GPU memory (via nvidia-smi dmon) and system memory usage
#
...
# CSV header
echo "timestamp,gpu_util_%,gpu_mem_util_%,gpu_mem_used_mb,gpu_mem_free_mb,gpu_temp_c,gpu_power_w,gpu_sm_clock_mhz,sys_mem_total_mb,sys_mem_used_mb,sys_mem_free_mb,sys_mem_available_mb,sys_mem_percent,container_mem_mb,container_mem_percent" > "$OUTPUT_FILE"
...
while true; do
    ...
    # Get Docker container memory usage (find any running nvidia container)
    # Using docker stats with --no-stream for single snapshot
    CONTAINER_MEM=$(docker stats --no-stream --format "{{.MemUsage}}" 2>/dev/null | grep -v "CONTAINER" | head -n1 | awk -F'/' '{print $1}' | sed 's/[^0-9.]//g')
    CONTAINER_MEM_MB=${CONTAINER_MEM:-"-1"}

    # Try to get container memory percentage
    CONTAINER_MEM_PCT=$(docker stats --no-stream --format "{{.MemPerc}}" 2>/dev/null | grep -v "CONTAINER" | head -n1 | sed 's/%//g')
    CONTAINER_MEM_PCT=${CONTAINER_MEM_PCT:-"-1"}

    # Write combined stats
    echo "${TIMESTAMP},${GPU_STATS},${MEM_TOTAL},${MEM_USED},${MEM_FREE},${MEM_AVAILABLE},${MEM_PERCENT},${CONTAINER_MEM_MB},${CONTAINER_MEM_PCT}" >> "$OUTPUT_FILE"
